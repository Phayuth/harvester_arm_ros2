...
from planner_manipulator.manipulator_planner import PlannerManipulator
...
class HarvestBridge:
    ...
    def srv_execute_motion_forward(path, jointName, request, response):
        ...
        for i in range(len(path)):
            ...
            point.positions = [path[i].config[0, 0],
                               path[i].config[1, 0],
                               path[i].config[2, 0],
                               path[i].config[3, 0],
                               path[i].config[4, 0],
    ...
    def srv_execute_motion_reverse(path, jointName, request, response):
        ...
        for i in range(len(path)):
            point = JointTrajectoryPoint()
            point.positions = [path[(i * -1) - 1].config[0, 0],
                               path[(i * -1) - 1].config[1, 0],
                               path[(i * -1) - 1].config[2, 0],
                               path[(i * -1) - 1].config[3, 0],
                               path[(i * -1) - 1].config[4, 0],
                               path[(i * -1) - 1].config[5, 0]]
            point.time_from_start = Duration(sec=1 + i, nanosec=0)
            trajMsg.points.append(point)
    ...
    def pub_virtual_state(timeNow, qCurrent, qAux, qGoal):
        ...
        jointMsgCurrent.position = [qCurrent[0], qCurrent[1], qCurrent[2], qCurrent[3], qCurrent[4], qCurrent[5]]
        ...
        jointMsgAux.position = [qAux[0], qAux[1], qAux[2], qAux[3], qAux[4], qAux[5]]
        ...
        jointMsgGoal.position = [qGoal[0], qGoal[1], qGoal[2], qGoal[3], qGoal[4], qGoal[5]]
